...
def calculate_jit(ohlcv_data, indicator_params, indicator_load):
    ...
    out_shape = (indicator_params.shape[0], ohlcv_data.shape[0],
                 total_indicator_cols)

    # 创建NaN数组
    out_arrays = np.full(out_shape, np.nan, dtype=np_float)

    cpu_parallel_calc_jit(ohlcv_data, indicator_params, indicator_load,
                          out_arrays)
    return out_arrays
...
def calculate_njit(ohlcv_data, indicator_params, indicator_load):
    ...
    cpu_parallel_calc_njit(ohlcv_data, indicator_params, indicator_load,
                           out_arrays)
    return out_arrays
...
def calculate_cuda(ohlcv_data, indicator_params, indicator_load):
    ...
    threadsperblock = 128
    blockspergrid = (indicator_params.shape[0] +
                     (threadsperblock - 1)) // threadsperblock
    if blockspergrid == 0:
        blockspergrid = 1

    gpu_kernel_device[blockspergrid,
                      threadsperblock](gpu_data, gpu_indicator_params,
                                       gpu_indicator_load, gpu_out_arrays)
    cuda.synchronize()
...
@timer
def precompiled(ohlcv_data_full, config, data_size=None):
    ...
    if ohlcv_data_full.flags["C_CONTIGUOUS"]:
        ohlcv_data = np.ascontiguousarray(
            ohlcv_data_full[:data_size]).astype(np_float)
        print("预编译数据转换为 C-contiguous 布局。")
    elif ohlcv_data_full.flags["F_CONTIGUOUS"]:
        ohlcv_data = np.asfortranarray(
            ohlcv_data_full[:data_size]).astype(np_float)
