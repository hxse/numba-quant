...
def create_params(config):
    ...
    np_params = np.array(
        [[
            p["value"] for i in _c["indicatros"] if i["active"]
            for p in i["params"]
        ] for _c in config],
        dtype=np_float,
    )
    ...
    result_name = [
        i["result_name"] for i in config[0]["indicatros"] if i["active"]
    ]
    ...
    segmented_params = extract_segmented_2d_array_by_load_params(
        np_params, np_load)
